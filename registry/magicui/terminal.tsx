...
export function Terminal({ children, className, sequence = true, startOnView = true }: { children: React.ReactNode; className?: string; sequence?: boolean; startOnView?: boolean }) {
    ...
    content = list.map((child) => {
      if (React.isValidElement(child)) {
        if (child.type === TypingAnimation) {
          const txt: string = typeof child.props.children === 'string' ? child.props.children : ''
          const speed: number = child.props?.duration ?? 60
          const d = delayAcc
          delayAcc += txt.length * speed
          return React.cloneElement(child as React.ReactElement<any>, { delay: d } as any)
        }
  ...
  return (
    <div ref={ref} className={cn('rounded-2xl border border-white/15 bg-black/40 backdrop-blur-sm text-white font-mono text-sm', className)}>
